...
def AddBreezeCard(cardnum):
	...
	sql_check_2 = 'SELECT BreezecardNum FROM Breezecard WHERE BreezecardNum="{}" AND BreezecardNum NOT IN (SELECT BreezecardNum FROM Conflict);'.format(cardnum)
	connection = pymysql.connect(host='academic-mysql.cc.gatech.edu',
								user = 'cs4400_Group_110',
								password = 'KAfx5IQr',
								db = 'cs4400_Group_110')
...
def AssignCardToOwner(cardNumber, newOwner):
	...
	try:
		with connection.cursor() as cursor:
			...
			n = cursor.fetchone()
			if n and n[0]==1:
				sql_newCardInsert = 'INSERT INTO Breezecard VALUES ("{}", 0.00, "{}");'.format(GenerateNewCardNumber(), curOwner)
